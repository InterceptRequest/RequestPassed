		...
		function load(currentPage,rowsPerPage){
			$.ajax({
				...
				data:{
					...
					$("#page").bs_pagination({
						currentPage: page.currentPage,        	// 打开网站显示的页码
						rowsPerPage: page.rowsPerPage,      	// 页面显示的数据个数
						maxRowsPerPage: page.maxRowsPerPage,    // 页面最多可以显示多少条数据
						totalRows: page.totalRows,      		// 总  数据  个数
						totalPages: page.totalPages,     		// 一共多少页
						visiblePageLinks: page.visiblePageLinks,// 一次显示多少个分页按钮
						// 翻页时触发的函数
						onChangePage: function (event, data) {
							load(data.currentPage, data.rowsPerPage);//页码 和  每页显示多少数据
						}
					});
				}
			})
					...
					<div class="form-group">
						<div class="input-group">
							...
							<select name="source" class="form-control">
								...
								<c:forEach items="${dicMapList.source}" var="v">
					...
					<div class="form-group">
						<div class="input-group">
							...
							<select name="state" class="form-control">
								<option value="">--请选择--</option>
								<c:forEach items="${dicMapList.clueState}" var="v">
									<option value="${v.value}">${v.text}</option>
								</c:forEach>
							</select>
						</div>
